...
# Marker-based total rewrite of the RETURN block v5.233
# Guaranteed unique identifiers
start_text = '  const formatDistance = (distanceKm: number) => {'
end_text = '// --- Helper Components for Virtualization ---'

s_idx = text.find(start_text)
e_idx = text.find(end_text)

# Find 'return (' after start_text
actual_start = text.find('  return (', s_idx)
...
middle = r'''  return (
    <div className="flex flex-col gap-10 w-full max-w-full overflow-x-hidden min-h-screen px-4 lg:px-8 bg-transparent">
      {/* 🚀 ELITE DASHBOARD HEADER v5.233 🚀 */}
      <header className={clsx(
        "rounded-[3.5rem] p-12 shadow-2xl border transition-all duration-1000 relative overflow-hidden backdrop-blur-[100px] border-white/5",
        isDark ? "bg-[#151B2C]/90 shadow-[#000000_20px_50px_80px]" : "bg-white/80 border-black/5"
      )}>
        <div className="absolute inset-0 bg-gradient-to-br from-blue-600/10 via-fuchsia-600/10 to-pink-600/10 opacity-50 mix-blend-overlay pointer-events-none" />
        
        <div className="relative z-10 flex flex-col xl:flex-row items-center justify-between gap-12">
          <div className="flex items-center gap-10">
            <div className={clsx(
              "w-24 h-24 rounded-[2.5rem] flex items-center justify-center shadow-[0_30px_60px_rgba(37,_99,_235,_0.4)] transition-all hover:rotate-12 hover:scale-110 active:scale-90 text-white bg-gradient-to-br",
      ...
      <>
        <div className="flex flex-col xl:flex-row gap-12 items-start min-h-[700px] mb-40">
          ...
          <div className="w-full lg:w-[480px] lg:sticky lg:top-12" data-tour="courier-select">
            <div className={clsx(
              ...
              <div className="relative z-10 flex flex-col h-full">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-10">
                    <div className={clsx(
                      "p-8 rounded-[2.5rem] border-2 flex flex-col items-center justify-center transition-all bg-gradient-to-b shadow-2xl",
                       isDark ? "from-white/10 to-transparent border-white/10" : "from-gray-50 to-white border-gray-200"
                    )}>
                ...
                {/* COURIER LIST INTERFACE (HEIGHT CONSTRAINED) */}
                <div className="p-8">
          ...
          {/* MAIN WORKING AREA: ORDER DASHBOARD (STABILIZED) */}
          <div className="flex-1 w-full relative min-w-0" data-tour="order-select">
            {!selectedCourier ? (
              <div className={clsx(
                "flex flex-col items-center justify-center p-32 lg:p-60 rounded-[5rem] border-8 border-dashed transition-all duration-1000",
                isDark ? "bg-[#151B2C]/40 border-white/10 shadow-[inset_0_5px_80px_rgba(0,0,0,0.6)]" : "bg-gray-50 border-gray-200"
              )}>
                <div className={clsx(
                  "w-48 h-48 rounded-[4rem] flex items-center justify-center mb-12 shadow-[0_50px_150px_rgba(37,_99,_235,_0.5)] relative group/main-icon",
                  isDark ? "bg-gray-900/95" : "bg-white"
                )}>
                   <div className="absolute inset-0 bg-blue-500/30 rounded-[4rem] blur-[60px] animate-pulse group-hover/main-icon:blur-[100px] transition-all" />
                   <TruckIcon className={clsx("w-24 h-24 relative z-10 transition-transform group-hover/main-icon:scale-110", isDark ? "text-gray-600" : "text-gray-300")} />
                </div>
                <h3 className={clsx("text-6xl font-black mb-6 tracking-tighter uppercase", isDark ? "text-gray-600" : "text-gray-400")}>
                  Виберіть кур'єра
                </h3>
                <p className={clsx("text-lg font-black uppercase tracking-[0.8em] opacity-30 animate-pulse", isDark ? "text-white" : "text-black")}>
                  ПЛАНУВАННЯ МОВЧКИ
                </p>
              </div>
            ) : (
              <div className="space-y-12 animate-in fade-in zoom-in duration-1000 slide-in-from-right-[50px]">
                <Suspense fallback={<div className="h-screen flex items-center justify-center opacity-30 font-black uppercase tracking-[0.8em] animate-pulse">SYSTEM LOADING...</div>}>
                  <OrderList 
        ...
        {/* Global Overlays & Modals Systems v5.233 */}
        <div id="km-stable-overlays-v5233" className="relative z-[200]">
...
# Replacement
final_content = text[:actual_start] + middle + text[e_idx:]

with open(path, 'w') as f:
    f.write(final_content)

print('DEPLOYED STABLE v5.233.')
